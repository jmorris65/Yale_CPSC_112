...
public class Tour { 
   ...
   public void insertNearest(Point p) {
      if (head == null) {
         ...
         double distance = 0;
         for (Node insert = head; insert.getNext() != null; insert = insert.getNext()) {
   ...
   public void shortestDetour(Point p) {
      if (head == null) {
         ...
         double distance = 0;
         for (Node insert = head; insert.getNext() != null; insert = insert.getNext()) {
            Node original = insert.getNext();
            Node placeholder = new Node(p, original);
            insert.setNext(placeholder);
            double move = distance();
            if (insert == head) distance = move;
            else if (move < distance) distance = move;
            insert.setNext(original);
         }
         for (Node insert = head; insert.getNext() != null; insert = insert.getNext()) {
            Node original = insert.getNext();
            Node placeholder = new Node(p, original);
   ...
   public void draw(Point p) {
      if (head == null)
         return;
      for (Node image = head; image.getNext() != null; image = image.getNext()) {
